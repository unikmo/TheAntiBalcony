...
export default async function AdminDashboard() {
  ...
  const actionOrders = data.orders.filter((o: any) => ["manual_review","needs_changes","failed","unavailable"].includes(o.status));
  const upcoming = data.orders.filter((o: any) => o.event_date && !["delivered","cancelled","failed"].includes(o.status)).sort((a: any,b: any) => String(a.event_date).localeCompare(String(b.event_date))).slice(0,12);
  const proofQueue = data.orders.filter((o: any) => ["played","capture_required","capture_processing","capture_ready","packaging_required","packaging"].includes(o.status));
  const cardQueue = data.cards.filter((o: any) => !["delivered","cancelled","failed"].includes(o.status));
  return (
    <>
      ...
      <section className="admin-grid-two">
        <article className="admin-panel">
          ...
          {actionOrders.length ? <div className="admin-list">{actionOrders.slice(0,10).map((o:any)=><Link key={o.id} href={`/admin/orders/${o.id}`}><div><strong>{o.order_ref}</strong><span>{o.customer_name || o.startup_name} · {o.occasion || label(o.tier)}</span></div><b>{label(o.status)}</b></Link>)}</div> : <p className="admin-muted">No Times Square exceptions right now.</p>}
        </article>
      ...
      <section className="admin-panel" id="times-square">
        ...
        <div className="admin-table"><div className="admin-tr admin-th"><span>Order</span><span>Date</span><span>Window</span><span>Customer</span><span>Package</span><span>Status</span></div>{upcoming.map((o:any)=><Link className="admin-tr" key={o.id} href={`/admin/orders/${o.id}`}><span><strong>{o.order_ref}</strong></span><span>{o.event_date}</span><span>{o.preferred_window_code || "—"}</span><span>{o.customer_name || o.startup_name}</span><span>{o.tier === "video" ? "$549 · Show + Keep" : "$399 · Show It"}</span><span><b className="admin-pill">{label(o.status)}</b></span></Link>)}</div>
      </section>

      <section className="admin-grid-two">
        <article className="admin-panel">
          <div className="admin-panel-head"><div><p>PROOF + DELIVERY</p><h2>After the screen</h2></div><span>{proofQueue.length}</span></div>
          <div className="admin-list">{proofQueue.slice(0,10).map((o:any)=><Link key={o.id} href={`/admin/orders/${o.id}`}><div><strong>{o.order_ref}</strong><span>{o.event_date} · {o.capture_provider || "capture pending"}</span></div><b>{label(o.status)}</b></Link>)}</div>
        </article>
        <article className="admin-panel" id="cards">
          ...
          {cardQueue.length ? <div className="admin-list">{cardQueue.slice(0,10).map((o:any)=><Link key={o.id} href={`/admin/cards/${o.id}`}><div><strong>{o.order_ref}</strong><span>{o.customer_name} · {o.card_count} card{o.card_count === 1 ? "" : "s"} · {money(Number(o.amount_total_cents))}</span></div><b>{label(o.status)}</b></Link>)}</div> : <p className="admin-muted">No curated-card orders yet.</p>}
        </article>
      </section>
    </>
  );
