...
export default async function AdminCardPage({ params, searchParams }: { params: Promise<{ id:string }>; searchParams: Promise<{updated?:string}> }) {
  const {id}=await params; const {updated}=await searchParams; const data=await getAdminCardOrder(id); if(!data) notFound(); const o:any=data.order;
  return <>
    ...
    <section className="admin-panel"><div className="admin-panel-head"><div><p>AUDIT TRAIL</p><h2>Card events</h2></div><span>{data.events.length}</span></div><div className="admin-events">{data.events.map((e:any)=><article key={e.id}><time>{new Date(e.created_at).toLocaleString("en-GB")}</time><strong>{label(e.event_type)}</strong><span>{e.source} · {label(e.status)}</span></article>)}</div></section>
  </>;
}
