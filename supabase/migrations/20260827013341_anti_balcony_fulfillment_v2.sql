create table if not exists public.anti_balcony_orders (
  ...
  status text not null default 'creative_upload_pending' check (status in ('creative_upload_pending','availability_check','creative_review','needs_changes','payment_pending','booked','scheduled','played','capture_required','capture_processing','capture_ready','packaging_required','packaging','proof_ready','delivered','cancelled','failed')),
  payment_status text not null default 'not_requested' check (payment_status in ('not_requested','pending','manual_paid','waived','refunded')),
  creative_path text,
  ...
  render_job_id text,
  render_callback_token_hash text check (render_callback_token_hash is null or char_length(render_callback_token_hash) = 64),
  render_error text,
  ...
  check ((alternative_window_start is null and alternative_window_end is null) or (alternative_window_start is not null and alternative_window_end is not null and alternative_window_end = alternative_window_start + interval '1 hour')),
  check ((scheduled_window_start is null and scheduled_window_end is null) or (scheduled_window_start is not null and scheduled_window_end is not null and scheduled_window_end = scheduled_window_start + interval '1 hour'))
);
...
create table if not exists public.anti_balcony_order_events (
  ...
  status text,
  source text not null check (source in ('customer','system','operations','blindspot','capture','shotstack','email','the-anti-balcony')),
...
create index if not exists anti_balcony_orders_ring_idx on public.anti_balcony_orders (ring_id, created_at desc);
create index if not exists anti_balcony_orders_status_idx on public.anti_balcony_orders (status, created_at asc);
create index if not exists anti_balcony_orders_email_idx on public.anti_balcony_orders (lower(email), created_at desc);
create index if not exists anti_balcony_order_events_order_idx on public.anti_balcony_order_events (order_id, created_at asc);
...
create policy "anti_balcony_orders_deny_direct_access" on public.anti_balcony_orders for all to anon, authenticated using (false) with check (false);
drop policy if exists "anti_balcony_order_events_deny_direct_access" on public.anti_balcony_order_events;
create policy "anti_balcony_order_events_deny_direct_access" on public.anti_balcony_order_events for all to anon, authenticated using (false) with check (false);
...
values
  ('anti-balcony-creative','anti-balcony-creative',false,262144000,array['image/jpeg','image/png','image/webp','video/mp4','video/quicktime','video/webm']::text[]),
  ('anti-balcony-capture','anti-balcony-capture',false,1073741824,array['image/jpeg','image/png','image/webp','video/mp4','video/quicktime','video/webm']::text[]),
  ('anti-balcony-deliverables','anti-balcony-deliverables',false,536870912,array['image/jpeg','image/png','image/webp','video/mp4']::text[])
