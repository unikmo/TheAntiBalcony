...
export function PopHome() {
  return <PopShell>
    <section className="pop-hero pop-wrap">
      <p className="pop-eyebrow">For the moments that move us.</p>
      <h1>Celebrate it. Show it.<br /><em>Keep it.</em></h1>
      <p className="pop-intro">Your celebration. A film to feel it again.<br />A UNIKMO card to keep it close.</p>
      <Link className="pop-button" href="/launch">Make it your moment ↗</Link>
      <figure className="pop-hero-image"><Image src="/unikmo-lady.webp" alt="A woman enjoying a memory with her UNIKMO card" width={1672} height={941} sizes="(max-width: 720px) 100vw, 1120px" priority /><figcaption>Some moments deserve more than a message.</figcaption></figure>
    </section>
    ...
    <section id="packages" className="pop-wrap pop-section">
      ...
      <div className="pop-offers">
        {(["free", "keep", "nasdaq"] as PopOffer[]).map((offer) => <article id={offer} className={`pop-offer ${offer === "keep" ? "pop-offer-featured" : ""}`} key={offer}>
          ...
          <Link className={`pop-button ${offer !== "keep" ? "pop-button-outline" : ""}`} href={`/launch?offer=${offer}`}>{POP_OFFERS[offer].action} ↗</Link>
